 ...
 * @LastEditTime: 2024-01-17 01:30:28
 * @Description: 常用时间方法
 */
...
export function getDayOfWeek(language: string = 'en'): string {
  ...
  export function formatDate(date: Date, formatString: string): string {
    ...
    if (re.test(dateShape)) {
      ...
      if (match) {
        const t = match[0];
        dateShape = dateShape.replace(t,
          (`${date.getFullYear() }`).substring(FOUR - t.length));
      }
    }
    for (const key in dateParts) {
      if (Object.prototype.hasOwnProperty.call(dateParts, key)) {
        ...
        if (regx.test(dateShape) && match) {
          const t = match[0];
          dateShape = dateShape.replace(t,
            t.length === 1 ? dateParts[key].toString() : (`00${ dateParts[key]}`).substring((`${ dateParts[key]}`).length).toString());
        }
      }
    }
    return dateShape;
   ...
   */
  export function isTimeInInterval(startTime: Date, endTime: Date, includeBoundaries: boolean = true): boolean {
